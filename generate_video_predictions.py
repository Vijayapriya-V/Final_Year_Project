...
output_csv = 'predicted_video_captions.csv'
max_length = 30
beam_width = 3
...
with open(output_csv, 'w', newline='', encoding='utf-8') as f:
    writer = csv.writer(f)
    writer.writerow(['video', 'caption'])  # ✅ Match column header in ground truth
    for key in features.files:
        caption = beam_search_caption(features[key], tokenizer, max_length, beam_width)
        writer.writerow([key + '.mp4', caption])  # ✅ Append .mp4 to filename
        print(f"✅ {key}.mp4: {caption}")
